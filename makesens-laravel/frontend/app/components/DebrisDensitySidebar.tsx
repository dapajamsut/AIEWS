// frontend/app/components/DebrisDensitySidebar.tsx
"use client";
...
interface DebrisDensitySidebarProps {
  densityPercentage: number;        // 0-100
  densityLevel: "Low" | "Medium" | "High";
  recommendation: string;
  lastAnalysis: string;
  totalObjects?: number;            // opsional, bisa ditampilkan juga
}

export function DebrisDensitySidebar({
  densityPercentage,
  densityLevel,
  recommendation,
  lastAnalysis,
  totalObjects,
}: DebrisDensitySidebarProps) {
  const densityColor = {
    ...
    <Card className="p-5 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
      <div className="space-y-4">
        <div className="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-3">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">Debris Density Index (DDI)</h3>
        </div>
        ...
        <div className="space-y-3">
          ...
          {totalObjects !== undefined && (
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-medium text-gray-900 dark:text-white">{totalObjects}</span>
            </div>
          )}
          <div className="flex justify-between items-center">
