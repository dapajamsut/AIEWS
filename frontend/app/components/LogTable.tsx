...
export function LogTable({ loggingInterval = 0, refreshTrigger = 0 }: LogTableProps) {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/logs?date=${date}&from=${from}&to=${to}`,
  ...
  const fetchExports = useCallback(async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/logs/exports`, {
        headers: { apikey: "pikel2" }
      });
